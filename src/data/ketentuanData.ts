const ketentuanData = [
  {
    dayId: 0,
    events: [
      {
        title: "PRA-OWEEK",
        rules: [
          "Kegiatan Pra O-WEEK akan dibagi menjadi kloter 1 dan 2.",
          "Trainee WAJIB mengikuti kegiatan Pra O-Week yang diselenggarakan secara ONLINE via Zoom.",
          "Trainee WAJIB memperhatikan segala informasi dan materi yang disampaikan selama kegiatan Pra - Oweek untuk mengerjakan tes yang akan diberikan.",
          "Trainee WAJIB mengerjakan tes Mental Health Assessment, Mental Health Course, Student Development Assessment, Career Interest Assessment, dan membuat Linkedin",
        ],
      },
      {
        title: "Ketentuan Zoom",
        rules: [
          "Trainee WAJIB hadir 15 menit sebelum kegiatan dimulai.",
          "Setiap trainee yang join WAJIB rename dengan FORMAT sebagai berikut: (no kelompok) - (nama kelompok) - (nama lengkap)",
          "Trainee WAJIB menggunakan pakaian yang sopan dan rapi.",
          "Trainee WAJIB menyalakan kamera (on camera) sepanjang kegiatan berlangsung.",
        ],
      },
    ],
  },
  {
    dayId: 1,
    events: [
      {
        title: "UPACARA",
        rules: [
          "Kegiatan Upacara 17 Agustus dan Opening Ceremony hanya boleh diikuti oleh Trainee.",
          "Trainee WAJIB mengikuti keseluruhan kegiatan secara OFFLINE di Universitas Ciputra.",
          "Trainee wajib membawa dan menggunakan Name Tag Trainee yang telah di print dan dimasukan ke lanyard.",
          "Dresscode : Kemeja Putih, Celana Panjang Hitam Kain (tidak sobek & tidak ketat), Sepatu bebas.",
        ],
      },
      {
        title: "Ketentuan Parkir",
        rules: [
          "Trainee diperbolehkan untuk membawa sepeda motor dengan menggunakan helm dan membawa SIM.",
          "Trainee yang membawa mobil dihimbau untuk mengisi mobil dengan minimal 4 trainee dalam satu mobil.",
          "Trainee yang membawa mobil wajib membawa dan memiliki SIM.",
          "Trainee yang membawa motor dihimbau untuk memarkirkan kendaraan di area PARKIRAN MOTOR Universitas Ciputra.",
          "Trainee yang membawa mobil dihimbau untuk memarkirkan kendaraan di area PARKIRAN LAPANGAN Universitas Ciputra.",
        ],
      },
      {
        title: "OPENING CEREMONY",
        rules: [
          "Kegiatan Upacara 17 Agustus dan Opening Ceremony hanya boleh diikuti oleh Trainee.",
          "Trainee WAJIB mengikuti keseluruhan kegiatan secara OFFLINE di Universitas Ciputra.",
          "Trainee wajib membawa dan menggunakan Name Tag Trainee yang telah di print dan dimasukan ke lanyard.",
          "Dresscode : Kemeja Putih, Celana Panjang Hitam Kain (tidak sobek & tidak ketat), Sepatu bebas.",
        ],
      },
      {
        title: "SEMINAR 5 Offline",
        rules: [
          "Trainee akan mendapatkan materi: Hygiene & Safety, BMC, dan Ethics, Social Media dan Financial Report",
          "Trainee WAJIB terbagi secara rata ke dalam 5 seminar dengan materi yang berbeda-beda.",
          "Trainee WAJIB mengikuti kegiatan seminar sesuai dengan materi yang telah didapatkan.",
          "Trainee WAJIB membawa dan menggunakan Name Tag Trainee yang telah dicetak dan dimasukkan ke dalam lanyard.",
          "Dresscode: Kemeja putih, celana panjang hitam berbahan kain (tidak sobek & tidak ketat), sepatu bebas.",
        ],
      },
      {
        title: "5 Seminars Online",
        rules: [
          "Trainee akan mendapatkan materi: Social Media dan Financial Report",
          "Trainee WAJIB terbagi secara rata ke dalam 5 Seminar dengan materi yang berbeda-beda",
          "Trainee WAJIB mengikuti kegiatan 5 Seminar sesuai dengan materi yang didapatkan",
          "Trainee WAJIB hadir tepat waktu pada zoom meeting",
          "Trainee WAJIB ON CAM saat melakukan zoom meeting ",
          "Trainee WAJIB  menggunakan Name Tag Trainee yang telah di print dan dimasukan ke lanyard.",
          "Dresscode : Kemeja Putih, Celana Panjang Hitam Kain (tidak sobek & tidak ketat), Sepatu bebas.",
        ],
      },
    ],
  },
  {
    dayId: 2,
    events: [
      {
        title: "TM SELLING DAY",
        rules: [
          "Setiap kelompok WAJIB datang yang diwakilkan oleh 2 perwakilan trainee.",
          "Diusahakan perwakilan trainee adalah trainee yang memahami dan dapat mengoperasikan google sheets.",
          "Setiap trainee WAJIB berpakaian sopan, rapi, bersepatu, dan menggunakan celana panjang.",
        ],
      },
    ],
  },
  {
    dayId: 3,
    events: [
      {
        title: "Communication (COM)",
        rules: [
          "Format: Offline",
          "Lokasi: Ruang 1434, 1435",
          "Tanggal: 21 Agustus 2025",
          "Waktu: 08.00 - 15.00",
          "PIC Dosen: Danu - 081314203820",
          "Informasi Tambahan: Mahasiswa diwajibkan membawa alat tulis, botol minum, bekal makan siang, dan obat-obatan pribadi. Pakaian harus rapi dan bersepatu.",
        ],
      },
      {
        title: "Food Technology Program (FTP)",
        rules: [
          "Format: Offline",
          "Lokasi & Jadwal:",
          "19 Agustus (08.00-12.00): Ruang 422, 420",
          "20 Agustus (08.00-12.00): Ruang 422, 417, 420, 421, 633",
          "21 Agustus (08.00-15.00): Ruang 422, 417, 420, 421, 633",
          "PIC Dosen: Oki Krisbianto - 085931321613",
          "Informasi Tambahan: Membawa buku dan alat tulis, botol minum, bekal makan siang, obat-obatan pribadi, jas laboratorium, dan berpakaian sesuai standar UC.",
        ],
      },
      {
        title: "Fashion Design and Business (FDB)",
        rules: [
          "Format: Offline",
          "Lokasi, Jadwal & PIC:",
          "19 Agustus (10.00-15.00): Ruang 1733 (PIC: Janet - 085717331690)",
          "20 Agustus (08.00-15.00): Ruang 1633 (PIC: Soelistyowati)",
          "21 Agustus (08.00-15.00): Ruang 1633 (PIC: Soelistyowati)",
          "22 Agustus (08.00-12.00): Ruang 422, 417, 420, 421, 633 (PIC: Enrico dan Ciawita - 085706900669)",
          "Informasi Tambahan: Membawa alat tulis, buku gambar A4, gunting, lem kertas, serta berpakaian rapi dan bersepatu.",
        ],
      },
      {
        title: "Architecture (ARS)",
        rules: [
          "Format: Offline",
          "Lokasi: Ruang 1832",
          "Tanggal: 19 - 22 Agustus 2025",
          "Waktu: 08.00 - 16.00",
          "PIC Dosen: Melania - 087739697888, Christian Ticualo - +62 812-3004-118",
          "Informasi Tambahan:",
          "Perlengkapan Seni: A5 Sketchbook, pensil 2B, kuas bulat no. 4 & 9, kuas datar no. 4 & 9, kertas cat air A3/A4, palet, wadah air, kain lap, cat poster (merah, biru, kuning, putih, hitam).",
          "Pakaian: Nyaman dan rapi (kaos, celana panjang, sepatu, topi untuk kegiatan luar ruangan).",
          "Pribadi: Obat-obatan, botol minum, sunblock.",
        ],
      },
      {
        title: "International Management and Technology (IMT)",
        rules: [
          "Format: Offline",
          "Tanggal: 19 & 20 Agustus 2025",
          "Lokasi & Jadwal:",
          "08.00 - 15.00: Ruang 504, 507",
          "09.00 - 14.00: Ruang 505A, 5058, 508",
          "PIC Dosen: Mychael M. Engel",
          "Informasi Tambahan: Membawa alat tulis, botol minum, obat-obatan pribadi, serta berpakaian rapi dan bersepatu.",
        ],
      },
      {
        title: "International School of Business (ISB)",
        rules: [
          "Format: Online",
          "Tanggal: 21 Agustus 2025",
          "Waktu: 09.00 - 14.00 (Waktu Zoom terjadwal pukul 08.00 AM Jakarta)",
          "Akses Zoom:",
          "Tautan: LINK MENYUSUL",
          "Meeting ID: 934 3555 6194",
          "Passcode: isb@uc",
          "PIC Dosen: Indra Maryati, Christian",
          "Informasi Tambahan: Siapkan alat tulis dan buku catatan. Berpakaian rapi, celana panjang, dan bersepatu. Wajib sarapan sebelum acara.",
        ],
      },
      {
        title: "Visual Communication Design (VCD)",
        rules: [
          "Format: Offline",
          "Lokasi: Ruang 1931, 1933, Hall Lantai 19",
          "Jadwal:",
          "18 Agustus: 13.00 - 17.00",
          "19 - 22 Agustus: Sesi Pagi (08.00-12.00) & Sesi Siang (13.00-17.00)",
          "PIC Dosen: Pramesti, Saniscara - +62 817 6985677",
          "Informasi Tambahan:",
          "Peralatan: Perlengkapan seni (akan diinfokan), Smartphone.",
          "Pakaian: Nyaman dan sopan (Kaos, celana panjang/jeans, sepatu, topi untuk kegiatan luar ruangan).",
          "Pribadi: Obat-obatan, botol minum, sunblock, sudah sarapan dan membawa bekal makan siang.",
        ],
      },
      {
        title: "Creative Business (CBZ)",
        rules: [
          "Format: Offline",
          "Lokasi, Jadwal & PIC:",
          "19 Agustus (08.30-12.00): Ruang 107 (PIC: Victor Yuwono, Otje Wibowo)",
          "20 Agustus (09.00-12.00): Ruang 1032 (PIC: Irra Dewi)",
          "Informasi Tambahan (Dresscode):",
          "Pria: Pakaian rapi, celana panjang, bersepatu, rambut disisir rapi.",
          "Wanita: Pakaian rapi, celana panjang, bersepatu, rambut dicepol rapi.",
        ],
      },
      {
        title: "International Business Management - International Class (IBM-IC)",
        rules: [
          "Format: Online",
          "Tanggal: 20 Agustus 2025",
          "Waktu: 14.00 - 16.00",
          "Akses Meeting:",
          "Tautan: LINK MENYUSUL",
          "Meeting ID: 98493315378",
          "Password: 429044",
          "PIC Dosen: Lenny Gunawan, Sasannaditya Surya Wang",
        ],
      },
      {
        title: "International Business Management - Regular Class (IBM-RC)",
        rules: [
          "Format: Offline",
          "Lokasi: Auditorium",
          "Tanggal: 19 - 21 Agustus 2025",
          "Waktu: 08.20 - 10.50",
          "PIC Dosen: Irantha Hendrika Kenang, Daniel Joel Kairupan, Johan Pratama Siahaineina",
          "Informasi Tambahan: Mahasiswa diminta membawa laptop dan charger.",
        ],
      },
      {
        title: "Psychology (PSY)",
        rules: [
          "Format: Offline",
          "Lokasi: Ruang 2134 & 2135",
          "Tanggal: 19 Agustus 2025",
          "Waktu: 09.00 - 15.00",
          "PIC Dosen: Meilani Sandjaja (0858-5149-7477), Cita Tri Kusuma (0856-4058-4581)",
          "Informasi Tambahan: Membawa alat tulis, botol minum, bekal makan siang, obat-obatan pribadi, laptop, dan charger. Berpakaian rapi dan bersepatu.",
        ],
      },
      {
        title: "Hospitality and Tourism Business - Event (HTEB)",
        rules: [
          "Format: Online",
          "Tanggal: 20 Agustus 2025",
          "Waktu: 09.00 - 12.00",
          "Akses Meeting: e.uc.ac.idionline meeting/join/e36238ee-ef64-4ab6-9195-",
          "PIC Dosen: Christy Widyawati",
          "Informasi Tambahan: Dresscode kemeja putih berkerah dan celana panjang hitam. Siapkan buku catatan.",
        ],
      },
      {
        title: "Medicine (MED)",
        rules: [
          "Format: Offline",
          "Lokasi: Ruang 307-308",
          "Tanggal: 18 - 21 Agustus 2025",
          "Waktu: 08.00 - 15.00",
          "PIC Dosen: dr. Edwin, Sp.PD (081334246474), dr. Denys, Sp.EM (081283959515)",
          "Informasi Tambahan: Membawa alat tulis, buku catatan, dan botol minum pribadi. Berpakaian rapi, celana panjang, dan bersepatu. Wajib sarapan sebelum acara.",
        ],
      },
      {
        title: "Accounting (ACC)",
        rules: [
          "Format: Online",
          "Tanggal: 21 Agustus 2025",
          "Waktu: 09.00 - 12.00",
          "Akses Meeting: Akan diinformasikan melalui Zoom Meeting.",
          "PIC Dosen: Yopy Junianto (08998937921)",
          "Informasi Tambahan: Membawa catatan, alat tulis, dan botol minum pribadi. Berpakaian rapi, celana panjang, dan bersepatu.",
        ],
      },
      {
        title: "Dental Medicine (DEM)",
        rules: [
          "Format: Offline",
          "Lokasi: Ruang 606",
          "Tanggal: 18 & 19 Agustus 2025",
          "Waktu: 08.00 - 15.00",
          "PIC Dosen: drg. Danella (085207821111)",
        ],
      },
    ],
  },
  {
    dayId: 4,
    events: [
      {
        title: "PARENTS GATHERING",
        rules: ["-",
        ],
      },
    ],
  },
  {
    dayId: 5,
    events: [
      {
        title: "NEXT GEN VISIONARIES",
        rules: ["Trainee diperbolehkan untuk membawa sepeda motor dengan menggunakan helm dan membwa SIM.",
          "Trainee yang membawa mobil dihimbau untuk mengisi mobil dengan minimal 4 trainee dalam satu mobil.",
          "Trainee diwajibkan untuk membawa tumblr dan peralatan makanan O-Week 2025.",
        ],
      },
    ],
  },
  {
    dayId: 6,
    events: [
      {
        title: "Ketentuan Parkir",
        rules: [
          "Trainee diperbolehkan untuk membawa sepeda motor dengan menggunakan helm dan membawa SIM.",
          "Trainee yang membawa mobil dihimbau untuk mengisi mobil dengan minimal 4 trainee dalam satu mobil.",
        ],
      },
    ],
  },
  {
    dayId: 7,
    events: [
      {
        title: "Ketentuan Parkir",
        rules: [
          "Trainee diperbolehkan untuk membawa sepeda motor dengan menggunakan helm dan membawa SIM.",
          "Trainee yang membawa mobil dihimbau untuk mengisi mobil dengan minimal 4 trainee dalam satu mobil.",
        ],
      },
    ],
  },
  {
    dayId: 8,
    events: [
      {
        title: "Ketentuan Parkir",
        rules: [
          "Trainee diperbolehkan untuk membawa sepeda motor dengan menggunakan helm dan membawa SIM.",
          "Trainee yang membawa mobil dihimbau untuk mengisi mobil dengan minimal 4 trainee dalam satu mobil.",
        ],
      },
    ],
  },
  {
    dayId: 9,
    events: [
      {
        title: "Ketentuan Parkir",
        rules: [
          "Trainee diperbolehkan untuk membawa sepeda motor dengan menggunakan helm dan membawa SIM.",
          "Trainee yang membawa mobil dihimbau untuk mengisi mobil dengan minimal 4 trainee dalam satu mobil.",
        ],
      },
    ],
  },
  {
    dayId: 10,
    events: [
      {
        title: "Ketentuan Parkir",
        rules: [
          "Trainee diperbolehkan untuk membawa sepeda motor dengan menggunakan helm dan membawa SIM.",
          "Trainee yang membawa mobil dihimbau untuk mengisi mobil dengan minimal 4 trainee dalam satu mobil.",
        ],
      },
    ],
  },
  {
    dayId: 11,
    events: [
      {
        title: "HENOSIS FOOD FOYAGE",
        rules: [
          "Dresscode Selling Day: Trainee diwajibkan untuk menggunakan kaos Universitas Ciputra, celana panjang gelap tidak sobek, name tag, bersepatu (warna bebas) serta membawa tumblr dan peralatan makan O-Week 2025, Atribut/Aksesoris Negara (detail ketentuan menyusul)",
          "Setiap trainee wajib mengajak minimal 5 orang untuk datang ke Selling Day.",
          "Trainee diwajibkan untuk menggunakan kaos Universitas Ciputra, celana panjang gelap tidak sobek, name tag, bersepatu (warna bebas) serta membawa tumblr dan peralatan makan O-Week 2025.",
          "Trainee tidak diperbolehkan untuk membawa sepeda motor.",
          "Trainee dihimbau untuk membawa mobil dengan minimal 4 orang dalam satu mobil.",
          "Dresscode Loading in (Kamis, 28 Agustus 2025 22.30-01.00): Baju Kaos bebas (tidak crop/sleeveless/tembus pandang), Celana Panjang Gelap (tidak robek dan tidak ketat), Sepatu Sneakers (warna bebas)",
        ],
      },
      {
        title: "Line OA Selling Day: @891vvono",
        rules: [],
      },
    ],
  },
  {
    dayId: 12,
    events: [
      {
        title: "FLAME OF TRIUMPH",
        rules: [
          "Trainee WAJIB mengikuti kegiatan Flame of Triumph yang diselenggarakan secara OFFLINE di Universitas Ciputra.",
          "Trainee WAJIB membawa dan menggunakan Name Tag Trainee yang telah di print dan dimasukan ke lanyard.",
        ],
      },
      {
        title: "Dresscode",
        rules: [
          "Laki-Laki   : Kemeja Putih, Celana Panjang Kain Hitam, Sepatu Pantofel Hitam",
          "Perempuan : Kemeja Putih, Rok Hitam Panjang dibawah lutut, Flat Shoes Hitam",
          "Setiap trainee WAJIB membawa jas almamater",
          "Setiap trainee DIHIMBAU membawa Topi dan Sunblock",
        ],
      },
      {
        title: "Ketentuan Parkir",
        rules: [
          "Trainee tidak diperbolehkan untuk membawa sepeda motor dan mobil ke Venue Palimanan",
          "Trainee sangat DIHIMBAU untuk melakukan drop-off di Venue Palimanan",
        ],
      },
    ],
  },
];
export default ketentuanData;
